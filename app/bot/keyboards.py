...
def expense_actions(
    ...
    category_buttons = [
        InlineKeyboardButton(text=category.name, callback_data=f"cat:{expense.id}:{category.id}")
        for category in categories[:6]
    ]
    rows = [category_buttons[i : i + 3] for i in range(0, len(category_buttons), 3)]
    rows.append(
        [
            InlineKeyboardButton(text=tr(locale, "edit"), callback_data=f"edit:{expense.id}"),
